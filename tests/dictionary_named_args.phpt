--TEST--
dictionary functionality: named arguments
--SKIPIF--
<?php
...
function check($data, $dictionary, $quality, $mode)
{
    ...
    echo $quality, ' -- ', $modeName, ' -- ',
        var_export(
            (($output === false) ? false
             : brotli_uncompress(data: $output, dict: $dictionary) === $data),
            true
        ), PHP_EOL;
}

echo "*** Compression Quality ***", PHP_EOL;
$mode = BROTLI_GENERIC;
for (
...
echo "*** Dictionary NULL ***", PHP_EOL;
check($data, null, $quality, $mode);

echo "*** Invalid Compression Quality ***", PHP_EOL;
